...
def get_wordcloud(viz_name):
    ...
    try:
        ...
        if request.method == "POST" and request.is_json and 'communications' in params:
            ...
            if data.get('graph_data') and data['graph_data'].get('links'):
                ...
                for link in data['graph_data']['links']:
                    if link.get('source') == 'Nadia Conti' or link.get('target') == 'Nadia Conti':
                        nadia_links.append({
                ...
                communications = [{
                    'source': 'Nadia Conti' if event.get('is_sender') else event.get('other_party', ''),
                    'target': event.get('other_party', '') if event.get('is_sender') else 'Nadia Conti',
        ...
        logger.debug(f"Wordcloud params: filter_person={filter_person}, include_indirect={include_indirect}")
